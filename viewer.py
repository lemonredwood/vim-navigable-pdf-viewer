...
class PDFViewer:
    def __init__(self, master, pdf_path):
        ...
        self.zoom_in_button.pack(side=tk.BOTTOM)

        self.zoom_out_button = tk.Button(self.master, text="Zoom Out", command=self.zoom_out)
        self.zoom_out_button.pack(side=tk.BOTTOM)
    ...
    def show_page(self):
        for i in range(self.pages_to_display):
            ...
            if page_index < self.doc.page_count:
                ...
                zoomed_width = int(img.width * self.zoom_factor)
                zoomed_height = int(img.height * self.zoom_factor)
                img = img.resize((zoomed_width, zoomed_height))
    ...
    def zoom_in(self):
        self.zoom_factor *= 1.2  # You can adjust the zoom increment as needed
        self.show_page()

    def zoom_out(self):
        self.zoom_factor /= 1.2  # You can adjust the zoom decrement as needed
        self.show_page()
